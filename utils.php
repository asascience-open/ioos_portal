      ...
      foreach ($searchResults->children($gmdNs)->{'MD_Metadata'} as $mdMetadata) {
        ...
        foreach ($mdMetadata->children($gmdNs)->{'identificationInfo'} as $identificationInfo) {
          ...
          foreach ($identificationInfo->children($srvNs)->{'SV_ServiceIdentification'} as $serviceIdentification) {
            $m['services'][$serviceType] = array(
              'getCaps' => sprintf("%s",$serviceIdentification->children($srvNs)->{'containsOperations'}[0]->children($srvNs)->{'SV_OperationMetadata'}->children($srvNs)->{'connectPoint'}->children($gmdNs)->{'CI_OnlineResource'}->children($gmdNs)->{'linkage'}->children($gmdNs)->{'URL'});
            );
            ...
            $m['services'][$serviceType]['geographicBbox'] = array(
            ...
            $m['services'][$serviceType]['temporalBbox'] = array(
